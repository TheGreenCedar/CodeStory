...
#[cfg(test)]
pub(crate) fn with_production_copy_and_writable_failure<T>(
    fail_writable: bool,
    ...
    impl Drop for Restore {
        fn drop(&mut self) {
            OWNER_WRITABLE_FAILURE.set(self.0);
        }
    }
    let restore = Restore(OWNER_WRITABLE_FAILURE.replace(fail_writable));
    let result = with_clone_disabled(action);
    drop(restore);
    result
}
...
/// Stage a sealed component under the caller's generation retention lease.
pub(crate) fn stage_file(
    source: &Path,
    destination: &Path,
    cancelled: &dyn Fn() -> bool,
) -> Result<codestory_store::SealedStageStats> {
    let stats =
        codestory_store::stage_sealed_file(source, destination, cancelled).map_err(|error| {
            match error {
                codestory_store::StorageError::Cancelled => crate::index::RetrievalIndexCancelled {
                    boundary: "sealed component copy",
                }
                .into(),
    ...
    tracing::info!(
        ...
        "staged sealed retrieval component"
    );
    Ok(stats)
}
...
mod tests {
    ...
    #[test]
    fn copied_stage_cancellation_retains_the_retrieval_error_class() {
        let root = tempfile::TempDir::new().expect("tempdir");
        let source = root.path().join("source");
        let destination = root.path().join("destination");
        std::fs::write(&source, vec![1_u8; 3 * 1024 * 1024]).expect("source");
        make_file_immutable(&source).expect("seal source");
        let calls = AtomicUsize::new(0);
        let error = with_clone_disabled(|| {
            stage_file(&source, &destination, &|| {
                calls.fetch_add(1, Ordering::SeqCst) >= 2
            })
            .expect_err("cancel chunked copy")
        });
    ...
    fn staged_file_is_distinct() {
        ...
        stage_file(&source, &destination, &|| false).expect("stage");
    ...
    #[test]
    fn stage_refuses_an_existing_destination() {
        let root = tempfile::TempDir::new().expect("tempdir");
        let source = root.path().join("source");
        let destination = root.path().join("destination");
        ...
        let error =
            stage_file(&source, &destination, &|| false).expect_err("must refuse overwrite");
