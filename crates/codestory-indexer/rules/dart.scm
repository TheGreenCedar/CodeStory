...
;; Dart grammar 0.4 gives bare calls a dedicated method_invocation node.
;; Member, null-aware, cascade, and chained calls stay on their selector paths.
(method_invocation
...
;; The leading call in a chained expression retains the selector form.
...
(_
  ...
  (selector
    (argument_part
      (arguments)))) @call_any
{
  node @callee_any.node
  attr (@callee_any.node) kind = "UNKNOWN"
  attr (@callee_any.node) name = (source-text @callee_any)
  attr (@callee_any.node) start_row = (start-row @callee_any)
  attr (@callee_any.node) start_col = (start-column @callee_any)
  attr (@callee_any.node) end_row = (end-row @callee_any)
  attr (@callee_any.node) end_col = (end-column @callee_any)

  edge @callee_any.node -> @callee_any.node
  attr (@callee_any.node -> @callee_any.node) kind = "CALL"
  attr (@callee_any.node -> @callee_any.node) line = (start-row @call_any)
  attr (@callee_any.node -> @callee_any.node) call_syntax = "dart_member"
}
