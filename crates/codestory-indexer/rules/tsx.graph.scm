    ...
    property: (_) @callee_any) @call_any)
{
  ...
  attr (@call_any.node -> @call_any.node) kind = "CALL"
  attr (@call_any.node -> @call_any.node) line = (start-row @call_any)
  attr (@call_any.node -> @call_any.node) call_syntax = "ts_member"
}

;; JSX usage from function render paths
(function_declaration
  ...
  body: (statement_block
    (return_statement
      (parenthesized_expression
        (jsx_element
          (jsx_opening_element name: (_) @component))))))
{
  node @component.node
  attr (@component.node) kind = "FUNCTION"
...
;; JSX usage from method render paths
(method_definition
  name: (_) @caller_name
  body: (statement_block
    (return_statement
...
;; Override-like
(method_definition
  (override_modifier) @override_marker
  name: (_) @method_name)
{
  node @override_marker.node
  attr (@override_marker.node) kind = "UNKNOWN"
  attr (@override_marker.node) name = (source-text @method_name)
  attr (@override_marker.node) start_row = (start-row @override_marker)
  attr (@override_marker.node) start_col = (start-column @override_marker)
  attr (@override_marker.node) end_row = (end-row @override_marker)
  attr (@override_marker.node) end_col = (end-column @override_marker)

  edge @method_name.node -> @override_marker.node
  attr (@method_name.node -> @override_marker.node) kind = "OVERRIDE"
}
