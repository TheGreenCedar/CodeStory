  ...
  name: (name) @callee_any) @call_any
{
  ...
  attr (@call_any.node -> @call_any.node) kind = "CALL"
  attr (@call_any.node -> @call_any.node) line = (start-row @call_any)
  attr (@call_any.node -> @call_any.node) call_syntax = "php_member"
}
...
;; Construction: `new Type(...)` becomes a placeholder CALL named by the
;; callee TYPE text, marked with its own `php_new` call syntax so construction
;; specs can annotate it without touching member-call placeholders.
(object_creation_expression
  (name) @callee_any) @call_any
{
  node @call_any.node
  attr (@call_any.node) kind = "UNKNOWN"
  attr (@call_any.node) name = (source-text @callee_any)
  ...
  attr (@call_any.node -> @call_any.node) kind = "CALL"
  attr (@call_any.node -> @call_any.node) line = (start-row @call_any)
  attr (@call_any.node -> @call_any.node) call_syntax = "php_new"
}
