...
mod tests {
    ...
    // TEMPORARY INVESTIGATION TEST - remove before commit.
    fn probe_answer_json(freshness: bool, coverage: bool) -> Value {
        let mut answer = json!({
            "answer_id": "req-1",
            "prompt": "probe target",
        ...
        if freshness {
            answer.as_object_mut().unwrap().insert(
                "freshness".to_string(),
                json!({
                    "status": "fresh",
        ...
        if coverage {
            answer.as_object_mut().unwrap().insert(
                ...
                json!([{ "path": "src/lib.rs", "status": "indexed" }]),
            );
        }
        answer
    }
    ...
    #[test]
    fn probe_context_packet_emits_fields_the_schema_forbids() {
        let declared = CONTEXT_PACKET_SCHEMA.declared_property_names();
        println!("declared = {declared:?}");
        println!(
            "additionalProperties = {}",
            CONTEXT_PACKET_SCHEMA.to_json()["additionalProperties"]
        );

        for (freshness, coverage) in [(true, false), (true, true), (false, true)] {
            let dto: codestory_contracts::api::AgentAnswerDto =
                serde_json::from_value(probe_answer_json(freshness, coverage))
                    .expect("probe answer deserializes");
            let packet = crate::output::context_packet_json(&dto);
            let emitted = packet
                .as_object()
            ...
            let undeclared = emitted
                ...
                .collect::<Vec<_>>();
            println!(
                "freshness={freshness} coverage={coverage} emitted={emitted:?} UNDECLARED={undeclared:?}"
            );
        }
    }
}
