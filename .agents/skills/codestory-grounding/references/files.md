...
|------|---------|-----------------|
| Inventory | `target/release/codestory-cli(.exe) files --project . --format markdown` | Language counts, usable/partial index notes, and a capped file list. |
| Coverage check | `target/release/codestory-cli(.exe) files --project . --language rust --format json` | Machine-readable file rows with `indexed`, `complete`, `role`, and `error_count`. |
