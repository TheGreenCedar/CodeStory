# `context` - Deep Evidence Packet For One Target

Builds an opinionated context bundle around one concrete retrieval target. This is not a question-answering or chat command: it does not interpret natural-language questions. Use `search` first when you only need lightweight candidate discovery.
...
- Do not pass broad questions to `context`. Use `ground --why`, concrete `search --repo-text on --why` queries, and then `context --id <node_id>` for selected anchors.
