...
| Failure path | If the target is ambiguous or missing, run `search --project <target-workspace> --query WorkspaceIndexer`, then retry with `--id <node_id>` or `--file <fragment>`. | Avoids guessing which symbol the report describes. |
| Integration edge | Use explore after `search`; feed the resolved node id into `context --id`, `trail --id`, or `snippet --id` when the next step needs sharper evidence. | Converts broad search into a focused browser handoff. |
...
- If query resolution is `ambiguous`, do not let `explore` pick for you. Run
  `search --project <target-workspace> --query <query>`, then retry with `--id <node_id>`
  or `--file <fragment>`.
