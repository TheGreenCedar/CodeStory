...
- For architecture questions, broad natural-language `search` is discovery
  ...
  MCP `drill` tool.
- `symbol`, `trail`, and `snippet` require a resolvable graph target. Semantic
  suggestions and repo-text hits are discovery leads, not graph proof. When a
  result supplies one unambiguous stable identity, the agent may use that exact
  identity with `context`, `snippet`, or an explicit relation operation. Keep
  ambiguity visible rather than choosing by display name.
...
clues, not graph anchors. Follow only an exact returned path or stable identity
when the task needs source or relation evidence; do not broaden a clue into a
repository search. A missing excerpt or unavailable search diagnostic is not a
focused source gap.
