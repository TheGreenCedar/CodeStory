...
This ignored harness still uses legacy hit and scoring fields; it is not a
schema-3 search acceptance gate.
...
- Low MRR means the right anchor exists but noisy hits outrank it. Use
  `search --format json` on the failing query and inspect the returned evidence,
  status, and gaps before changing ranking.
...
- Field-qualified misses usually mean the candidate was retrieved but filtered
  ...
  `--format json` and compare source-backed evidence before weakening the
  filter.
