...
native backend metadata enter the protected scan by default. Only the explicit
corpus/proof harness list and test/fixture directories are excluded; missing
required protected or harness paths fail the lint so moves require review.

| Classification | Protected or allowed surface | Static contract |
| --- | --- | --- |
| Product launch and setup | Plugin manifests, hooks, MCP launcher, grounding skill guidance/setup scripts, Codex worktree setup, and the Windows installer | Protected; the configured directories are scanned recursively, while test and fixture directories are excluded. |
...
| Explicit corpus harnesses | Task manifests, packet/repo benchmark drivers and scorers, holdout provisioning, release-candidate measurement, the retrieval-sidecar contract workflow, and test fixtures | Allowed to load evaluation corpora; these paths are evidence producers, not product or release-decision logic. Other scripts and workflows remain protected by default. |
