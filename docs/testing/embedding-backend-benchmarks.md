...
| Generalization boundary guard, tainted-query quarantine, and cache replay block | `lint-retrieval-generalization.mjs` scans production Rust against benchmark manifests plus script and eval-only query/probe catalogs | Prevents production retrieval code and semantic docs from copying benchmark identities, prompts, claims, paths, or query/probe text | Release guard; historical packets created with the removed domain-alias catalog are invalidated | Required before new promotion evidence. |
