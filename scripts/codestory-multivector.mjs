...
import { greedyRows, validateRows } from './lib/multivector-evidence.mjs';
...
function run(directory) {
  ...
  const envelope = { input_sha256: hash(inputPath), source: identity(), executable: data.python,
    executable_sha256: hash(data.python), argv, started_at: new Date().toISOString() };
...
function validate(directory) {
  ...
  publish(path.join(directory, 'validation.json'), { status: 'validated', authority: data.authority,
    ...
    source: identity(), baselines: result.rows.map(r => r.baseline) });
}
